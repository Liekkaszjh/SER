...
class CTMAM(nn.Module):
    ...
    def forward(self, *input):
        ...
        x = self.bn5(x)
        x = F.relu(x)
        # ====== 强制对齐到 LCT 期望的空间尺寸 (6,14) ======
        if x.size(2) != 6 or x.size(3) != 14:
            x = F.adaptive_avg_pool2d(x, (6, 14))
        # ================================================
...
class CTMAM_EMODB(nn.Module):
    ...
    def forward(self, *input):
        ...
        # 2) 如果 xp 是 [B,1,H,W] 而 x 是 [B,C,H,W]，做一下通道广播（更稳）
        if xp.dim() == 4 and x.dim() == 4 and xp.size(1) == 1 and x.size(1) != 1:
            xp = xp.expand(-1, x.size(1), -1, -1)
